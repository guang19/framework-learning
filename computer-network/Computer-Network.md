...
<u>**总结起来就是:多次发送可能共用一个传输，一个发送可能多占用多个传输。**</u>

>其实归根到底，究其根本原因是:

<u>TCP是面向字节流的协议，消息之间没有边界。</u>
<u>而UDP虽然也可以一次性传输多个包或者多次传输一个包，</u>
<u>但UDP的每个消息都是有边界的，因此不会有粘包和半包问题。</u>
...
>一个URL只能访问到一个资源，
>所以URL有着和URI相同的作用:
>都可以作为资源的唯一标识符，但URL也有URI没有的作用:对资源的定位。
>
