...
单例模式:
[Singleton](https://github.com/guang19/framework-learning/blob/dev/design-pattern/src/main/java/com/github/guang19/designpattern/singleton)

单例模式主要有两种写法:懒汉式和饿汉式。
网上有的文章说 **"饿汉式的单实例在类加载阶段就实例化了，而懒汉式在第一次获取实例的时候才初始化。"**
这里我从Java/Jvm(其他语言暂且不论)的角度来反驳下这种观点，如果各位同学如果同意这种观点，且是从我写的[类的生命周期](https://qsjzwithguang19forever.gitee.io/framework-learning/gitbook_doc/jdk_jvm_juc-learning/%E7%B1%BB%E7%9A%84%E7%94%9F%E5%91%BD%E5%91%A8%E6%9C%9F.html)
...
**只有当我们主动使用类时，这个类才会被初始化，也就会初始化单例了。**
根据我查阅的资料，我总结了使用类主要涉及8种情况: 
...
综上所述，只考虑正常使用时初始化类的情况: 创建类的实例对象；**调用类的静态方法**；给类的静态变量赋值；访问类的静态变量 才会初始化类，
而**当我们访问饿汉式单例的时候，90%的情况都是通过静态方法获取单例，所以大部分情况都是当我们获取单例的时候才会初始化单例，
