...
NIO: Non Blocking IO        非阻塞IO(也称New IO 新IO)
AIO: Asynchronous IO        异步IO

````

````java
同步/异步 和 阻塞/非阻塞 这2组概念之间是一点关系都没有的，
请各位同学不用顺口说成 同步阻塞是， 异步非阻塞
````
...
#### BIO(Blocking IO)
>我们在开始学Java的时候都会接触到BIO，我们很快就可以编写出BIO的程序来，这也是它的
>一个优点。
>
>但越往后越会发现BIO的缺点，如文件IO，如果是大文件，那么一次IO的时间就会很长，调用
>IO的线程就会一直等待文件读取完成。这就是BIO的性能瓶颈所在。
>
>在网络编程中，IO资源是非常宝贵的。如果使用BIO来进行网络编程，那么读写操作都是阻塞的，
>即执行请求的线程，在请求没有执行完之前，线程无法继续执行其他请求。这样，服务端在同一时间内，
>只能处理一个客户端的请求。如果想处理多个客户端的请求，就只能为每个客户端分配一个线程，
>但是操作系统能分配的线程数是有限的，所以如果请求数增加，操作系统就很可能会崩溃。

![BIO网络模型](../img/BIO网络模型.png)

#####总结: BIO的优点:编码简单。 BIO的缺点:性能低下，如果要处理数量较多的网络IO请求，将消耗很多线程资源。

#### NIO(Non Blocking IO)
>NIO是JDK1.4引入的一种新的IO模型，称为非阻塞式的IO。
>
>NIO是基于事件驱动的模型来开发的。
>在NIO中，有一个负责接受所有请求的Selector线程，每个请求的socketChannel都会被注册到
>这个Selector上，Selector线程就会轮询检测Selector上socketChannel的事件。
>每当Selector线程检测到socketChannel有事件发生的时候，就会进行相应的处理。
>这就是NIO被称为非阻塞IO的原因，Selector线程不会阻塞在一个socketChannel上。
>如此一来，NIO使用单线程或少量线程处理多连接请求，它处理多任务的能力是比BIO
>好很多的。

![NIO网络模型](../img/NIO网络模型.png)

>但不可忽视的一个缺点是:NIO虽然是非阻塞的，但是当它处理数据库，文件IO这样的操作时，
>仍然会消耗较长时间，因为NIO的线程在执行方法时，方法如果是同步的，那么线程就需要阻塞等待
>方法的返回。
>
>NIO非阻塞体现在它接受请求的能力上，但在执行业务逻辑代码时，它仍然是阻塞的。
>
>所以到这，可以看出NIO的缺点:如果线程执行的业务逻辑方法是同步的，
>那么线程仍然会阻塞，一旦并发量增加，那么面对大量请求，耗费的时间也是很长的。
>并且编写NIO的程序代码比编写普通BIO的程序代码更加复杂。
