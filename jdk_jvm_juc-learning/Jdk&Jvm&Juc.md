...
- [java基础知识(部分图源:[JavaGuide](https://github.com/Snailclimb/JavaGuide))](#java-----------javaguide--https---githubcom-snailclimb-javaguide--)
    + [equals知识点](#equals---)
      - [== 和 equals区别](#-----equals--)
      - [hashCode方法返回的真是对象内存地址吗?](#hashcode---------------)
      - [equals方法重写要求](#equals------)
      - [为什么重写equals方法一定要重写hashcode方法?](#-----equals-------hashcode---)
    + [Java引用类型](#java----)
      - [强引用](#---)
      - [软引用](#---)
      - [弱引用](#---)
      - [虚引用(幽灵引用)](#---------)
      - [软引用,弱引用,虚引用在被GC前会被加入到与其关联的引用队列中.](#-------------gc----------------)
      - [进程和线程](#-----)
      - [线程的几种状态(见:jdk Thread类源码中的state枚举类)](#----------jdk-thread-----state----)
      - [并发和并行](#-----)
      - [sleep方法和wait方法](#sleep---wait--)
      - [stop,suspend,resume等方法为什么会被遗弃](#stop-suspend-resume----------)
      - [interrupt,interrupted,isInterrupted方法别搞混了](#interrupt-interrupted-isinterrupted------)
      - [join方法](#join--)
      - [yield方法](#yield--)
    + [对象在内存中的布局(64位)](#----------64--)
      - [对象头](#---)
        * [1. Mark Word(mark)](#1-mark-word-mark-)
        * [2. Klass Pointer /  Compressed Klass](#2-klass-pointer----compressed-klass)
      - [实例数据](#----)
      - [对齐填充](#----)
    + [synchronized](#synchronized)
      - [谈谈 synchronized 关键字](#---synchronized----)
      - [synchronized底层原理](#synchronized----)
      - [synchronized 使用方法](#synchronized-----)
        * [1. 修饰静态方法](#1-------)
        * [2. 修饰实例方法](#2-------)
        * [3. 修饰代码块](#3------)
      - [Synchronized和ReentrantLock的区别](#synchronized-reentrantlock---)
        * [1.Synchronized基于jvm层面,ReentrantLock基于java API层面.](#1synchronized--jvm---reentrantlock--java-api--)
        * [2.ReentrantLock提供了更高级的功能](#2reentrantlock---------)
      - [lock](#lock)
      - [tryLock](#trylock)
      ...
      - [volatile](#volatile)
    + [CAS](#cas)
      - [CAS在JAVA中的底层实现(Atomic原子类实现)](#cas-java-------atomic------)
          + [Unsafe类](#unsafe-)
        * [并且有个重要的细节就是,Atomic原子类内部的value值都是由volatile修饰的,这就使得Atomic的值是对其他线程可见的](#------------atomic------value----volatile--------atomic-----------)
        * [CAS的缺点](#cas---)
          + [1. 循环时间开销大](#1--------)
          + [2. 只能保证一个共享变量的原子操作](#2----------------)
          + [3. ABA问题](#3-aba--)
    ...
    + [AQS (AbstractQueuedSynchronizer)](#aqs--abstractqueuedsynchronizer-)
      - [AQS 概述](#aqs---)
        * [AQS的2种共享资源访问方式](#aqs-2---------)
          + [独占式(Exclusive)](#----exclusive-)
          + [共享式(Share)](#----share-)
        * [CountDownLatch原理](#countdownlatch--)
        * [Semaphore](#semaphore)
        * [CycliBarrier](#cyclibarrier)
      - [ReentrantReadWriteLock如何区分读写锁的?](#reentrantreadwritelock---------)
    + [线程池的好处](#------)
      - [线程池构造参数](#-------)
        ...
        * [handler](#handler)
        * [阿里巴巴开发者手册不建议开发者使用Executors创建线程池](#-----------------executors-----)
    + [java集合](#java--)
      - [HashMap](#hashmap)
      - [HashTable](#hashtable)
      - [TreeMap](#treemap)
      - [ArrayList与LinkedList](#arraylist-linkedlist)
      - [ArrayList和Vector](#arraylist-vector)
      - [Set](#set)
      - [ConcurrentModificationException异常](#concurrentmodificationexception--)
      - [线程安全的 List](#------list)
    ...
    + [Linux IO模型](#linux-io--)
      ...
      - [信号驱动IO(网络IO模型)](#----io---io---)
      - [异步IO](#--io)
    + [JVM(参考:《深入理解Java虚拟机》(周志明))](#jvm---------java----------)
      - [JVM运行时内存分区](#jvm-------)
      - [程序计数器](#-----)
        * [程序计数器的特点](#--------)
      - [Java虚拟机栈](#java----)
    ...
    + [GC](#gc)
      ...
      - [什么是GC Root ?](#---gc-root--)
    + [垃圾回收算法](#------)
      - [复制算法(Copying)](#-----copying-)
      - [标记-清除算法(Mark-Sweep)](#--------mark-sweep-)
      - [标记-整理算法(Mark-Compact)](#--------mark-compact-)
      - [分代收集算法](#------)
      - [内存分配与垃圾回收策略](#-----------)
      - [一次GC的过程](#--gc---)
      - [动态年龄阈值](#------)
    + [垃圾回收器](#-----)
      - [Serial 串行收集器](#serial------)
      - [Serial Old 串行收集器(老年代版本)](#serial-old-------------)
      - [Parallel Scavenge 并行多线程收集器](#parallel-scavenge---------)
      - [Parallel Old 并行收集器(老年代版本)](#parallel-old-------------)
      - [ParNew 多线程收集器](#parnew-------)
      - [CMS 并发标记清除收集器](#cms----------)
        * [CMS回收内存主要分为以下阶段:](#cms-------------)
      - [G1 收集器](#g1----)
        * [G1回收器的主要特点有:](#g1----------)
        * [G1收集器回收内存主要分为以下阶段](#g1---------------)
    + [JVM常用参数](#jvm----)
      - [堆栈相关](#----)
      - [GC相关](#gc--)
      - [其他](#--)
...
##### 1.Synchronized基于jvm层面,ReentrantLock基于java API层面.
