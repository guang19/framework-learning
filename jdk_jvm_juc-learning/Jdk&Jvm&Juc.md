...
<u>Java虚拟机栈描述的是Java方法运行时的内存模型。</u>
...
>方法区在JVM规范里是各个线程共享的一部分区域，它用于存储已被jvm加载的类的元信息，
>包括Class常量池等数据。
...
>元空间存储着已被加载的类的元信息，字符串常量池从永久代转移到了堆内存之中。
...
>一层一层的判断，最终可能会由BootstrapClassLoader判断。
...
>标记-清除算法分为2个步骤：标记和清除。
>
>首先标记出所有需要回收(不可达)的对象，在标记完成后，
统一回收所有被标记的对象。

标记-清除算法的缺点主要有2个:
2. 标记和清除2个阶段的耗时都比较长，可以总结为效率较低。
1. 对象在内存中的分布可能是不连续的，分散的，
...
>都是首先标记出所有需要被回收的对象。
>但标记之后并不直接清理被标记的对象，
>而是使仍然存活的对象向内存一端移动，然后清理掉这一端外的内存。
         
标记-整理算法的优点是:
几乎不会如标记-清除算法那样产生不连续的内存碎片。
但，所谓慢工出细活。标记-整理的效率大概率上都是比标记-清除要低的。         
