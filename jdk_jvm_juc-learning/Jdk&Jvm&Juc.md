...
##### 操作系统的内核：
````java
操作系统的内核是操作系统的核心部分。他负责管理系统的进程，设备(硬件)驱动程序，文件，内存，网络等部分。
````
...
>直到内核准备好数据并读取完数据(磁盘或网络等数据),
>然后将数据从内核复制到用户应用进程，最后应用程序再处理数据。

![BIO原理](../img/BIO原理.png)


>阻塞IO的阻塞体现在:
>用户应用进程等待内核准备数据并读取数据的这段时间和把数据从用户态拷贝到
...
>这个过程就叫轮询。轮询知道内核准备好数据，再从内核把数据拷贝到用户应用进程，
>再进行数据处理。
...
等待其他任务执行完毕才会去发起系统调用，这就意味着数据的延时读取。

#### 多路复用IO
>在多路复用IO模型中,用户应用进程会调用操作系统的select/poll/epoll函数,
>select/poll/epoll函数不同于系统调用,它会使内核同步检测指定的socket
>(在NIO,socket中就是注册到Selector上的SocketChannel,可以允许多个)
>是否有数据,直至内核监听的socket有数据才select/poll/epoll函数会返回。
