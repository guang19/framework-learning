...
准确来说进程与线程属于衍生关系。
进程是操作系统执行程序的一次过程,在这个过程中可能会产生多个线程。比如在使用QQ时，有窗口线程，文字发送的线程，语音输入的线程，可能不是很恰当，但是就是这个意思。
在Java中，一个进程产生的多个线程共享这个进程内的堆，metaspace等共享资源，每个线程又有自己的程序计数器，虚拟机栈，本地方法栈。
...
#### 偏向锁:
>偏向锁与轻量级锁相似，都适用于多线程无 锁竞争的环境。
>偏向锁:
>如名，偏向锁会偏向第一个获取它的线程，接下来如果没有其它线程获取这个锁，
>那么持有该锁的线程将无需再次获取这个锁，直接执行同步代码,如果有其他线程竞争这个锁，
>那么偏向锁将会升级为轻量级锁。
>
>偏向锁适用于单线程无锁竞争环境
