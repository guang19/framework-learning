...
二级索引:
![B+树二级索引](../img/database/B+树二级索引(辅助索引).png)
...
![Mysql表文件截图](../img/database/Mysql索引文件截图.png)

聚集索引和非聚集索引:

![B+树聚集索引](../img/database/B+树索引.png)
...
覆盖索引:
![B+树覆盖索引](../img/database/B+树覆盖索引.png)
...
![联合索引(多列索引)](../img/database/联合索引(多列索引).png)

无论是否符合最左前缀原则，每个字段的索引都生效:

![联合索引生效](../img/database/联合索引之查询条件生效.png)
...
![事务的四大特性](../img/database/事务的四大特性.png)
...
- **隔离性(Isolation)**: 隔离性指事务与事务之间是相互独立的，互补干扰的，
一个事务的失败，不会影响另一个事务。
...
![读已提交&不可重复读](../img/database/读已提交&不可重复读.png)
...
**可以看出不可重复读侧重于数据的不一致，也就是数据的修改UPDATE。**
...
![可重复读](../img/database/可重复读.png)

* **幻读**: 幻读与不可重复读非常相似，都是指事务在没有结束之前，多次读取的结果不一致。
**但不可重复读是读取一条记录的结果不一致，而幻读是读取结果的记录数不一致。**

>>假设某张表的一个字段为money，有2条记录。
>>一个事务A读取了这2条记录，此时，另一个事务B对表中的记录做了DELETE或INSERT，
>>使得事务A再次读取的时候，发现表中的记录只有1条或者有3条。

**可以看出幻读的侧重在于记录的数量，也就是记录的DELETE或INSERT。**

在Mysql中将隔离级别设置为 REPEATABLE READ即可以避免不可重复读，也可以避免
幻读:

![可重复读解决解决幻读](../img/database/可重复读解决解决幻读.png)
